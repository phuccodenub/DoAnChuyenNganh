...
import { useLessonAiChat } from '@/hooks/useAi';
...
export function AiAssistantCard({ courseTitle, courseDescription, lessonTitle, lessonId }: AiAssistantCardProps) {
  ...
  const handleAsk = async () => {
    ...
    try {
      const res = await lessonChat.mutateAsync({
        message: userMsg,
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Hỏi về nội dung bài học này.</p>
        </div>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="Hỏi trợ lý AI về nội dung bài học..."
            rows={3}
          ...
          <div className="flex justify-end">
            <Button onClick={handleAsk} disabled={lessonChat.isPending}>
              {lessonChat.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
              Hỏi trợ lý
            </Button>
          </div>
        </div>
