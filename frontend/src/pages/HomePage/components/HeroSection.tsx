  ...
  return (
    <section id="hero" className="border-b border-slate-100 bg-gradient-to-b from-white via-slate-50/60 to-white">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-16 lg:grid-cols-[0.4fr_0.6fr] lg:items-center">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-green-50 px-1.5 py-1 text-xs font-semibold text-green-600">
            <span className="rounded-full bg-gradient-to-r from-green-400 via-teal-500 to-sky-800 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.32em] text-white">
              NEW
            </span>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            <Button
              ...
              className="group rounded-full bg-gradient-to-r from-green-400 via-teal-500 to-sky-800 px-10 py-3 text-sm font-semibold text-white shadow-[0_12px_24px_-12px_rgba(79,70,229,0.55)] transition hover:from-green-500 hover:via-teal-600 hover:to-sky-900 sm:px-4 sm:py-3.5"
            >
            ...
            >
              <span className="flex items-center gap-2.5">
                ...
                <Play className="h-4 w-4 rounded-full border border-slate-200 bg-slate-50 p-1 text-slate-500 transition group-hover:border-green-200 group-hover:text-green-500" />
              </span>
            </Button>
          </div>
          ...
          {isAuthenticated && (
            <p className="mt-4 text-sm text-slate-500">
              Chào mừng trở lại, <span className="font-semibold text-slate-700">{user?.first_name} {user?.last_name}</span>. Tiếp tục hành trình học tập của bạn.
            </p>
          )}
        </div>
