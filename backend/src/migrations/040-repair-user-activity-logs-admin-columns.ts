...
/**
 * Repair migration for environments where migration 038 is marked executed,
 * but the physical schema is missing the admin columns/indexes.
 */
export async function up(queryInterface: QueryInterface): Promise<void> {
  ...
  try {
    await queryInterface.sequelize.query(`UPDATE user_activity_logs SET status = 'success' WHERE status IS NULL;`);
  } catch (error) {
