...
export class PrerequisiteService {
  ...
  async getPrerequisites(courseId: string, userId?: string, userRole?: string): Promise<any[]> {
    try {
      ...
      if (!course) {
        throw new ApiError('Course not found', RESPONSE_CONSTANTS.STATUS_CODE.NOT_FOUND);
      }
      ...
      if (!isOwner && !isAdmin && !isPublished) {
        throw new ApiError('Not authorized to view prerequisites', RESPONSE_CONSTANTS.STATUS_CODE.FORBIDDEN);
      }
  ...
  async createPrerequisite(
    ...
    try {
      ...
      if (!isOwner && !isAdmin) {
        throw new ApiError('Not authorized to manage prerequisites', RESPONSE_CONSTANTS.STATUS_CODE.FORBIDDEN);
      }

      // Check if prerequisite course exists
      const prerequisiteCourse = await Course.findByPk(data.prerequisite_course_id);
      if (!prerequisiteCourse) {
        throw new ApiError('Prerequisite course not found', RESPONSE_CONSTANTS.STATUS_CODE.NOT_FOUND);
      }
      ...
      throw new ApiError((error as Error).message, RESPONSE_CONSTANTS.STATUS_CODE.BAD_REQUEST);
    }
  }
  ...
  async bulkCreatePrerequisites(
    ...
    try {
      ...
      if (prerequisiteCourses.length !== prerequisiteIds.length) {
        throw new ApiError('Some prerequisite courses not found', RESPONSE_CONSTANTS.STATUS_CODE.NOT_FOUND);
      }
  ...
  async deletePrerequisite(
    ...
    try {
      ...
      if (!prerequisite) {
        throw new ApiError('Prerequisite not found', RESPONSE_CONSTANTS.STATUS_CODE.NOT_FOUND);
      }

      if (prerequisite.course_id !== courseId) {
        throw new ApiError('Prerequisite does not belong to this course', RESPONSE_CONSTANTS.STATUS_CODE.BAD_REQUEST);
      }
