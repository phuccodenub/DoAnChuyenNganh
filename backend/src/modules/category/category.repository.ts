...
export class CategoryRepository extends BaseRepository<CategoryInstance> {
  ...
  protected getModel(): ModelStatic<CategoryInstance> {
    return Category as unknown as ModelStatic<CategoryInstance>;
  }

  async findBySlug(slug: string): Promise<CategoryInstance | null> {
    const model = this.getModel();
    return await model.findOne({ where: { slug } as WhereOptions<CategoryAttributes> });
  }
  ...
  async safeDelete(id: string): Promise<void> {
    ...
    const subCount = await model.count({ where: { parent_id: id } as WhereOptions<CategoryAttributes> });
    if (subCount > 0) {
